#!/usr/bin/env python3
"""Local JSON data source for the iStat Pro compatibility shim."""
...
def snapshot_monitor(server: ThreadingHTTPServer) -> None:
    while True:
        if LAST_SNAPSHOT_AT and time.time() - LAST_SNAPSHOT_AT > SNAPSHOT_TIMEOUT:
            print("No /snapshot requests received recently; shutting down iStat Pro data server")
            server.shutdown()
            return
        time.sleep(1.0)
...
def build_cpu() -> list:
    ...
    if current and len(current) == len(PREVIOUS_CPU_TICKS):
        ...
        return [[system, user, nice, idle], per_core[:16]]
    ...
    per_core = [[total_busy] for _ in range(min(cores, 16))]
    return [[system, user, nice, idle], per_core]
...
def build_memory() -> list:
    ...
    pageins = values.get("Pageins", 0) * page_size
    pageouts = values.get("Pageouts", 0) * page_size
    swap = run(["sysctl", "-n", "vm.swapusage"])
    swap_match = re.search(r"used = ([\d.]+)([MGT])", swap)
    swap_used = f"{swap_match.group(1)} {swap_match.group(2)}B" if swap_match else "0 MB"
    return [
        format_bytes(free),
        format_bytes(total),
        format_bytes(active),
        format_bytes(inactive),
        format_bytes(wired),
        percent,
        [format_bytes(pageins), format_bytes(pageins)],
        [format_bytes(pageouts), format_bytes(pageouts)],
        "",
        swap_used,
    ]
...
def build_battery() -> list:
    ...
    source = "Battery" if "Battery Power" in output else "Power Adapter"
...
def snapshot() -> dict:
    ...
    process_count = len(run(["ps", "-axo", "pid"]).splitlines()) - 1
    return {
        "cpu": build_cpu(),
        "memory": build_memory(),
        "disks": build_disks(),
        ...
        "uptime": build_uptime(),
        "load": build_load(),
        "processCount": str(max(process_count, 0)),
    }
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Serve local iStat Pro widget data.")
    ...
    import threading
    monitor = threading.Thread(target=snapshot_monitor, args=(server,), daemon=True)
    monitor.start()
    print(f"iStat Pro data server listening on http://{HOST}:{PORT}/snapshot")
